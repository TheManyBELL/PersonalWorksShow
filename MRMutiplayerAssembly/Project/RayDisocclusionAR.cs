...
public class RayDisocclusionAR : MonoBehaviour
{
    ...
    // qinwen code
    private MirrorController mirrorMyController;
    ...
    private void AddNewSegment(int index)
    {
        ...
        for (int i = 0; i < MaxBezierCurveCount + 2; i++)
        {
            ...
            t_straightLineCurve[i].layer = LayerMask.NameToLayer("DepthCameraUnivisible");
        ...
        for (int i = 0; i < MaxstraightLinesCount; i++)
        {
            ...
            t_straightLine[i].layer = LayerMask.NameToLayer("DepthCameraUnivisible");
        ...
        for (int i = 0; i < 2; i++)
        {
            t_arrow[i] = CreateNewLine("Segment_" + index.ToString() +
                "_arrow_" + i.ToString());
            t_arrow[i].layer = LayerMask.NameToLayer("DepthCameraUnivisible");
    ...
    private bool GetSamplePointsVisibility()
    {
        ...
        for (int i = 0; i < VisibilitySampleCount; ++i)
        {
            ...
            {
                linesVisibility[segmentIndex][i] = true;
                continue;
            }
            ...
            linesVisibility[segmentIndex][i] = (minDepth >= screenP.z);
            if (completeConvered)
            {
                completeConvered = !linesVisibility[segmentIndex][i];
            }
            if (i == VisibilitySampleCount - 1)
            {
    ...
    {
        Vector3 faceP = MWorldToScreenPointDepth(p);
        float lastMinDepth = GetDepthScript.GetDepth((int)faceP.x, (int)faceP.y);
        ...
        while (inScreenRange(faceP + new Vector3(direction.x, direction.y)))
        {
            ...
            float minDepth = GetDepthScript.GetDepth((int)faceP.x, (int)faceP.y);
